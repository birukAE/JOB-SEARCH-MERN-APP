...
const CATA_REGEX =/^[a-zA-Z0-9 ]{1,12}$/;
function Memberreg() {
...
setValiddate(DATE_REGEX.test(date));
  ...
  useEffect(() => {
    ...
    setValidcatagory(CATA_REGEX.test(catagory));
    }, [catagory])
 useEffect(() => {

      setErrMsg('');
  }, [username, password, comfirm,phone,transaction,date ,payer,pack,bank,fullname,other,email,upcv,tempo,catagory])
...
Axios.post("http://localhost:3006/memberregs/mempost",
 {username:username,
password:password,email:email,fullname:fullname,upcv:fileName,cvsta:cvsta,tempo:fileName1,bank:bank,
pack:pack,date:date,payer:payer,transaction:transaction,phone:phone,other:fileNames,comfirm:comfirm,catagory:catagory})
.then((response)=>{
...
onBlur={() => setBankFocus(false)}
  ...
  Allowed special characters: <span aria-label="exclamation mark">!</span> <span aria-label="at symbol">@</span> <span aria-label="hashtag">#</span> <span aria-label="dollar sign">$</span> <span aria-label="percent">%</span>
    ...
    <Button disabled={!validName||!validpassword||!validFullname ||!validBank || !validPayer|| !validTransaction ||!validDate||!validEmail||!validcatagory ? true : false}onClick={Submit}>submit</Button>
    </Form>
